...
2. **按词条数量计算毕业率**
   ...
   - 用户不手动区分首、副词条；系统在 8 个首词条、32 个副词条中自动寻找占用首词条最少的合法分配，并实时显示 `首词条 N/8`、`副词条 N/32`、`普通词条 N/40`。
   - 同一属性在每件装备上最多占一个副词条，允许同件装备的首、副词条同属性；首词条池和副词条天然部位池会联合校验，不把转律视为额外来源。
   - 每行上限随其他词条动态变化；非法增加只回退当前词条并说明冲突。旧组合按原字段顺序迁移，后出现的冲突词条自动降到合法数量。
