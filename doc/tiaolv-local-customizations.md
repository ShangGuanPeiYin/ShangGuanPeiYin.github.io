...
| 手动毕业率词条数量模式 | 在“毕业率分析 → 手动填写”中增加“按词条数量 / 直接填写面板”双模式。数量模式可选择“全部按满值”或“全部按承音值”，不区分主副词条，普通词条合计最多 40 条；同时按装备部位约束特殊词条：对首领单位增伤最多 2 条、全武学增效最多 2 条、任一具体武器武学增效最多 1 条。三率、五维、当前流派攻击和神力以快捷专栏展示，神力只显示当前具体流派的武器增效，专栏底部可直接选择精准弓、会心弓或会意弓并同步当前方案；PVE 隐藏玩家增效，单体/群体奇术增伤等少见项收进“其他词条”。每次计算都在毕业率弹窗右侧同步展示“词条数量最终面板”，复用最佳配装的字段分组和数值格式，并在精准率、会心率、会意率后显示“溢出 N% 白值”；离开手动页、切回直接填写或关闭弹窗时自动收起。每个角色、流派及流派版本可保存最多 50 个命名词条组合，支持切换、更新、重命名、删除和未保存改动提示，切换后实时重算毕业率。两种输入模式使用相互隔离的结果节点，数量模式在完整计算后同步渲染，原手填防抖任务无法覆盖其结果。使用 8 件理论金装换算最终面板，定音沿用当前装备或现有贷款定音设置，并隐藏下方最终面板输入。切回直接填写时重新显示并恢复之前的手填面板，配置保存在原手动面板记录中并随完整备份迁移。 |
...
2. **按词条数量计算毕业率**
   ...
   - 普通词条最多 40 条、不区分主副词条；对首领单位增伤最多 2 条、全武学增效最多 2 条、每种具体武器武学增效最多 1 条，旧组合载入时也会自动收敛到合法上限。
...
主要保护标记：`FULL_BACKUP_KIND`、`MANUAL_STAT_COUNT_CONFIG_KEY`、`grad-manual-stat-category-panels`、`grad-manual-stat-preset-select`、`grad-manual-stat-count-result`、`writeManualPanelInputs(container, panel, false)`。
