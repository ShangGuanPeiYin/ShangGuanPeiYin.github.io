...
| 大小外均可流标签 | `buildFlowTypeTagHtml`、`.flow-type-tag` | 装备卡片底部流派标签抽成 `buildFlowTypeTagHtml`：大外流橙底 `#ff9800`、小外流青底 `#00bcd4` 保持不变，新增 `不限制`（大小外均可）中性灰底 `#6b7280`，文案为「大/小外流」；未知/空 flowType 不渲染标签。 |
