...
| 完整 JSON 备份 | 在导出/导入弹窗中动态插入 `下载完整备份` 按钮；单个明文 JSON 备份全部角色，以及每个角色的装备（保留 ID）、全部流派方案、转律状态和毕业率手动面板数据。格式标识为 `yysls-tiaolv-full-backup`、版本为 `schemaVersion: 3`（新增装备 `flowType` 字段），不包含可重建的计算缓存。 |
| 完整 JSON 恢复 | 在导出/导入弹窗中动态插入 `恢复完整备份` 文件控件；导入前校验格式、角色/装备/方案结构和装备引用，汇总展示角色数、装备数、方案数及同名角色，确认后按角色覆盖并保留未包含的本地角色；写入失败时回滚。继续兼容旧版 `1.2` 单角色装备 JSON。导入 `schemaVersion: 2` 备份时自动为装备补全 `flowType: "大外流"`。 |
...
1. **武器专属转律词库**
   - `weapon1`、`weapon2` 通过 `isWeaponTransmutationEquip` 与 `filterTransmutationTargetsForEquip` 使用武器候选过滤：最大鸣金、最大裂石、最大牵丝、最大破竹攻击必须排除，最大无相攻击必须保留。
   - 非武器部位继续走原有当前流派大属攻过滤；不得修改 `CommonData.TRANSMUTATION_POOLS` 作为实现手段。
   - 该过滤必须同时覆盖装备编辑弹窗目标列表、`GradModal.getTransmutationVariants()`、转律建议和最佳配装自动转律，避免入口之间词库分叉。
2. **可转律默认排除项**
   - `DEFAULT_EXCLUDED_TRANSMUTATION_STATS` 包含四种大属攻、最大无相攻击与会意率。首次开启可转律时默认不选，但用户可手动勾选；既有 `excludedTargets` 恢复后不得被默认值覆盖。
...
| 大外流/小外流标签 | `flowType`、`inferFlowType`、`handleSaveEquip`、装备卡片底部 | 新增 `flowType: "大外流" | "小外流" | "不限制"` 字段。录入弹窗新增单选下拉框（默认"大外流"）。新建装备时自动推断（≥2条最小外功攻击→小外流；≥2条最大外功攻击或劲→大外流；否则不限制），编辑时用用户选择。装备卡片底部 class pills 左侧显示橙色大外流/青色小外流标签，不限制不显示。 |
| 最佳配装外功流向筛选 | `bestbuild-flowtype-filter`、`bestBuildFlowType`、`findBestBuild` | 忽略可用流派限制未勾选时显示流向筛选下拉框（大外流/小外流/不限制，默认大外流）。勾选忽略时隐藏。筛选逻辑：不限制时全部装备参与；指定方向时只允许该方向或不限制的装备参与（旧装备无 flowType 视为大外流）。 |
| 大小外均可流标签 | `buildFlowTypeTagHtml`、`.flow-type-tag`、`resolveEquipFlowType` | 装备卡片底部流派标签抽成 `buildFlowTypeTagHtml`：大外流橙底 `#ff9800`、小外流青底 `#00bcd4` 保持不变，新增 `不限制`（大小外均可）中性灰底 `#6b7280`，文案为「大/小外流」；未知/空 flowType 不渲染标签。缺少 `flowType` 的旧装备由 `resolveEquipFlowType` 推断，并**保留推断出的「不限制」**，不再强制改写为大外流。 |
| 库筛选包含大小外均可 | `matchesFlowTypeFilter`、`flowTypeFilterDB` | 装备库「大小外」筛选改为与最佳配装口径一致：筛选大外流或小外流时，`不限制` 装备以及缺少 `flowType` 的旧装备都照常显示；仅排除另一单一流向。 |
...
| 装备卡片转律提示仅显示已勾选目标 | `buildZhuanlvTargetHint`、`excludedTargets` | 已转律装备卡片上括号内的可转词条提示在原有部位限制与重复词条排除之外，再排除该装备 `excludedTargets` 中未勾选的目标；无剩余目标时不显示括号提示。不改变转律编辑器勾选、存储或候选搜索。 |
| 转律目标列表实时刷新与分槽位记忆 | `buildZhuanlvTargetView`、`resolveZhuanlvExcludedTargets`、`readSubStatsByRow`、`_zhuanlvExcludedBySlot` | 目标词条列表改为按当前表单（按行索引快照）计算“自身词条/重复词条”，编辑已有装备时改动词条下拉值会立即刷新，不再沿用存档副词条；同一弹窗会话内各副词条槽位各自记住自己的目标勾选（槽位草稿优先，其次匹配槽位的存档，最后默认排除集）。存档仍只保存当前选中的 active 槽位。 |
| 最佳配装弓箭多选 | `normalizeBestBuildBowTypes`、`bestBuildBowTypes`、`bestbuild-bow-type-check` | 取消原「考虑不同弓箭套装」单一复选框，改为精准弓/会心弓/会意弓三个独立复选框，可任选 1～3 个，默认仅勾选当前弓；至少保留一个，取消全部时回退当前弓。所选集合并入默认遍历与智能遗传搜索的弓箭候选池、结果缓存键与遗传种子，每套结果仍记录实际使用的 `bowType`。 |
| 最佳配装遍历剩余时间 | `formatBestBuildEta`、`best-build-progress-text` | 默认遍历进度在“已检查 N/M 种组合”行末尾追加“预计剩余 X”，由累计平均速度估算；未知速度显示 `--`，不足 1 秒显示“不到 1 秒”。仅改默认遍历，不影响转律建议、遗传搜索与培养方向进度。 |
